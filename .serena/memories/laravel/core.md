...
- **`GET /api/categories?similarity_query=...` quota**: 로그인 사용자 + `similarity_query` 있을 때 `api_quota_remaining` 1 차감. 관리자 우회, 비로그인 사용자는 추천 API 미지원(세션 불필요) — quota 차감 없음. quota 0 → 429. `similarity_query` 없이 목록 조회만 할 때는 차감 없음.
