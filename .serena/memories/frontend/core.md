...
- **마이페이지 `/mypage`**: 독립 경로. 서버 컴포넌트에서 `auth_token` 쿠키 → `getUser(token)` → 미인증 시 `/login?redirect=/mypage`. 헤더 닉네임 `<Link href="/mypage">`. 구성: API key 관리(`api-key-section`·`api-key-card`·`api-key-create-dialog`), 사용량 대시보드(`usage-dashboard` — "전체 한도" 표기, 일일 리셋 아님), 차트(`usage-chart`), 이력(`usage-history`).
- **관리자 회원 관리**: URL 기반 라우팅 — `admin/layout.tsx`에서 `Link` + `usePathname()`. MENU: `/admin`(시스템 설정), `/admin/member`(회원 관리). 각 페이지 `page.tsx`에서 서버 측 `redirect("/login?redirect=...")`로 인증 게이트 — `searchParams`로 쿼리 파라미터 보존. `page-content.tsx`에는 인증 리다이렉트 로직 없음. `user-management.tsx` → `fetchAdminUsers()`로 회원 목록. `user-detail-modal.tsx`에서 상세 + `quota-adjust-dialog.tsx`로 쿼타 조절. **⚠️ 응답 구조**: 백엔드 평탄 구조 → `setDetail(res.data).
