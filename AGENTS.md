...
- **`#[Hidden]` 모델 필드 + accessor 패턴** — `#[Hidden(['key'])]` 등으로 JSON 직렬화에서 제외된 필드를 프론트엔드에서 직접 참조하면 `undefined` 에러 발생. **해결**: 백엔드에서 `$appends` + `Attribute` accessor로 안전한 미리보기 값 제공 (예: `key_preview`로 잘린 키 표시). 프론트엔드 타입에도 optional + preview 필드 추가.
