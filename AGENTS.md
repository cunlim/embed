...
- **`search` 파라미터 다국어 검색** — `GET /api/categories?search=...`는 `category_name_ko`·`category_name_en`·`category_name_zh`·`category_code` 네 필드를 LIKE(`%검색어%`) 부분 검색. 검색 모드에서 사용. `batchStatus()`도 동일 로직.
- **`search_lang` 파라미터 (분류선택 언어별 검색)** — `search_lang=ko|en|zh` 추가 시 해당 언어 컬럼에서만 **접두사 검색**(`검색어>%`) 수행. 분류선택 모드에서 `lang`에 따라 자동 전달. 미지정 시 기존 다국어 부분 검색 동작. 전파 체인: `embed-page-inner.tsx`(`hierarchyLangRef.current`) → `loadCategories()`(8번째 인자) → `getCategories()`(`search_lang` 쿼리 파라미터) → 백엔드 `index()`·`batchStatus()`. `page.tsx` SSR 프리페치에서도 `hierarchyLang` 전달.
