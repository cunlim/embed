...
- **작업 완료 전 검증** — `.claude/hooks/run-all-checks.sh` 실행하여 결과 확인 (항상 stdout 출력, `--terminal`은 상세 출력 강제). test-results/*.txt 파일은 항상 기록. tsc, lint, test, pint 모두 EXIT=0 확인 후 마무리.
