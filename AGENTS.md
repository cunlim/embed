...
- **API 필터 파라미터 전파 체인** — 새 파라미터 추가·리네임 시 frontend(api.ts → hooks → embed-page-inner.tsx → page.tsx → category-hierarchy.tsx), backend(모든 조회 컨트롤러), test mock 모두 수정 필요. `TaskExecution`의 `onStepsChange` → `loadCategories`도 동일 체인. 생성 API(`createCategory`)도 동일 — `api.ts` → `useCategories.addCategory()` → `embed-page-inner.tsx` 핸들러 체인 + 백엔드 `store()`·`CategoryStoreRequest`까지 전파. hook interface 타입(`UseCategoriesReturn`)도 갱신 필수. 한 곳만 수정하면 나머지는 이전 동작 유지. **리네임 시 추가 검증**: ①`window.history.pushState`·`replaceState` 직접 호출부(`updateURL` 바이패스), ②`fetchBatchStatus` 등 POST body 키(URL 쿼리 파라미터와 별도), ③Eloquent Resource 클래스(`$request->input()`), ④FormRequest validation 제약(`max` 등), ⑤오류 메시지 문자열 내 파라미터명, ⑥하위 호환성을 위한 이전 파라미터명 폴백.
...
- **`onFolderChange` URL 파라미터 보존** — 폴더 변경 시 `onFolderChange` 핸들러에서 URL을 수동 생성하면 기존 파라미터(`owner_scope` 등)가 소실됨. `updateURL({ folder, userId, pageNumber: 1 })`을 사용하여 기존 파라미터를 보존해야 함.
- **admin `user_id` 우선 규칙** — 조회·생성 모두 적용. 관리자가 URL에 `user_id`를 명시하면 `owner_scope=my`가 있어도 `user_id`를 우선 적용. 생성 시에도 `selectedUserId` → `addCategory(userId)` → API까지 전파.
...
- **`useCategories` mutation reload 컨텍스트 완전성** — `addCategory()`·`deleteCategory()` 내부 reload는 `currentPageNumber`·`currentPageSize`·`currentOwnerScope`·`currentLikeQuery`·`currentFolder` 모든 ref를 전달해야 함. 새 컨텍스트 파라미터 추가 시 ref 선언과 reload 호출 인자 양쪽 모두 업데이트.
...
- **batch `onComplete`·`onCategoryComplete` 콜백 패턴** — `onComplete`는 `filterRef.current`로 현재 필터를 읽고 `loadCategories(1, ...)` + `updateURL({ pageNumber: 1 })`로 URL 동기화. `onCategoryComplete`는 루프 중 `loadCategories` 호출 금지.
...
- **`steps` 필터 적용 시 회원 소유권 범위 제한** — 백엔드 `CategoryQueryService::applyUserScope()`에서 일반 회원이 `steps` (작업 실행) 필터를 적용하여 조회할 때, `owner_scope`가 `my`가 아니더라도(`all`이거나 미지정) `user_id = 1` (전체 공개 카테고리)은 수정 권한이 없으므로 조회에서 제외하고 본인 소유 카테고리(`user_id = $user->id`)만 반환하도록 제한.
