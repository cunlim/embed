...
- **인증 리다이렉트 URL 전체 보존 패턴** — Server Component에서 `redirect("/login?redirect=/path")`로 리다이렉트하면 쿼리 파라미터와 해시가 누락됨. 수정: (1) Server Component에서 인증 실패 시 리다이렉트하지 않고 `serverUser={null}`을 Client Component에 위임, (2) Client Component에서 `window.location.pathname + search + hash`로 전체 URL을 구성하여 `encodeURIComponent` 후 `/login?redirect=`에 포함. `LoginFormClient`에 클라이언트 측 인증 체크(`useAuth` + `router.replace`)를 추가하여 로그인 상태에서 로그인 페이지 접근 시 리다이렉트. 보호된 페이지의 `page-content.tsx`에서 인증 체크 시 이 패턴을 따라야 함.
- **관리자 페이지 서버 측 인증 가드** — `/admin/*` 페이지는 쿼리 파라미터·해시를 사용하지 않으므로, Server Component에서 `redirect("/login?redirect=/admin")`로 즉시 리다이렉트. Client Component에 위임하면 SSR 렌더링 후 클라이언트에서야 리다이렉트되어 header/nav 플래시가 발생. `app/admin/page.tsx`·`app/admin/member/page.tsx`에서 `!token` 또는 `getUser()` 실패 시 서버 측 `redirect()` 호출 필수.
