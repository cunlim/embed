...
## 4. 데이터베이스 주요 테이블 정의
* **categories:** (네이버 카테고리 기준 단일 테이블)
    * `id` (PK)
    * `category_code` (VARCHAR, 플랫폼 영문/숫자 코드)
    * `category_name_ko` (VARCHAR, 예: "가구/인테리어>DIY자재/용품>목재")
    * `category_name_zh` (VARCHAR, 중국어 번역본)
    * `category_name_en` (VARCHAR, 영어 번역본)
