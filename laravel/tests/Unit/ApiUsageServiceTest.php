...
test('log — API 사용 로그를 기록한다', function () {
    ...
    expect($log->parameters)->toBe(['text' => '청바지']);
...
test('getRecentHistory — 최근 사용 내역을 apiKey 관계와 함께 반환한다', function () {
    $this->service->log($this->apiKey->id, $this->user->id, '/api/v1/search', null, 200, 100);
    $this->service->log($this->apiKey->id, $this->user->id, '/api/v1/recommend', null, 200, 150);
    ...
    expect($endpoints)->toContain('/api/v1/recommend');
    // apiKey 관계가 로드되어야 함
