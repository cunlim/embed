...
test('recommendPaginated — keyword 필터를 적용해도 rank는 전체(user scope) 기준으로 계산된다', function () {
    $category1 = Category::factory()->create([
        'category_code' => '50000000',
        'category_name_ko' => '화장품/미용>헤어케어>샴푸',
        'user_id' => 1,
    ]);
    $category2 = Category::factory()->create([
        'category_code' => '50000001',
        'category_name_ko' => '식품>과일',
        'user_id' => 1,
    ]);

    // category1 (keyword match "화장품"): query와 직교 방향 → rank 2 (전체 중)
    $emb1_arr = array_fill(0, 1024, 0.0);
    $emb1_arr[1] = 1.0;
    $ce1 = new CategoryEmbedding;
    $ce1->category_id = $category1->id;
    $ce1->language = 'ko';
    $ce1->embed_model_name = 'bge-m3:latest';
    $ce1->embedding = new Vector($emb1_arr);
    $ce1->save();

    // category2 (keyword 불일치 "식품"): query와 동일 방향 → rank 1 (전체 중)
    $emb2_arr = array_fill(0, 1024, 0.0);
    $emb2_arr[0] = 1.0;
    $ce2 = new CategoryEmbedding;
    $ce2->category_id = $category2->id;
    $ce2->language = 'ko';
    $ce2->embed_model_name = 'bge-m3:latest';
    $ce2->embedding = new Vector($emb2_arr);
    $ce2->save();

    $query_arr = array_fill(0, 1024, 0.0);
    $query_arr[0] = 1.0;
    $searchLog = new SearchLog([
        'search_keyword' => '샴푸',
        'normalized_keyword' => '샴푸',
        'embed_model_name' => 'bge-m3:latest',
    ]);
    $searchLog->embedding = $query_arr;

    $service = new RecommendationService;

    // keyword '화장품'으로 필터 — category2(식품)는 결과에서 제외됨
    $result = $service->recommendPaginated($searchLog, 'ko', 20, 1, null, '화장품');

    // category1만 결과에 포함됨
    expect($result->total())->toBe(1);
    expect($result->items()[0]->id)->toBe($category1->id);

    // keyword 필터가 rank에 영향을 주지 않음 → category1은 전체 2개 중 2위 (필터 후 1위로 재설정되지 않음)
    expect($result->items()[0]->rank_ko)->toBe(2);
});
