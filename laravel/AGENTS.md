...
- **`CategoryProcessingService`** — step 실행, 텍스트 업데이트, 삭제 로직. `determineMissingSteps()`, `runStep()`, `updateText()`, `deleteWithEmbeddings()`, `batchRun()`.
...
- **PHP 8.5 호환성** — `maatwebsite/excel`는 PHP 8.5 미지원. `openspout/openspout` 사용.

### 배치/벌크 API

- **`POST /api/categories/batch-run`** — 서버 측 배치 실행. STEP_ORDER: `embedding.ko` → `translation.en` → `embedding.en` → `translation.zh` → `embedding.zh`. 재시도: 최대 2회 (지수 백오프 1s, 2s). 카테고리 간 실패 격리. `connection_aborted()`로 클라이언트 연결 해제 감지.
- **`POST /api/categories/bulk-upload`** — Excel 파일(xlsx) 업로드로 카테고리 일괄 등록. OpenSpout으로 파싱. 컬럼: `category_code`(선택), `category_ko`(필수), `category_en`(선택), `category_zh`(선택).
- **`GET /api/categories/bulk-download`** — 카테고리 Excel 다운로드. `response()->stream()`으로 스트리밍. `CategoryQueryService::buildListQuery()`로 동일 필터 적용.
- **`batchRun` 권한 처리** — ids 모드에서 `isAdmin() || user_id === current_user.id` 필터 적용. 비권한 카테고리는 자동 제외 (에러 아님).
