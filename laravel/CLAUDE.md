...
- php 8.5, Laravel 13, Pest 4, Pint 1, Sanctum 4, Socialite 5

=== Laravel Boost 도구 ===

- `search-docs` — 코드 변경 전 항상 먼저 실행. 버전별 문서 검색.
- `database-query` — 읽기 전용 SQL (tinker 대신 사용)
- `database-schema` — 마이그레이션/모델 작성 전 테이블 구조 확인
- `get-absolute-url` — URL 공유 전 체계·도메인·포트 확인

=== 핵심 규칙 ===

- 코드 변경 전 `search-docs`로 문서 확인. `php artisan make:` 명령어로 파일 생성. 기존 컨벤션 준수.
- `vendor/bin/pint --format agent`로 PHP 포맷팅. `php artisan test --compact`로 테스트 실행.
- 변경마다 테스트 작성/갱신 필수. 모델 생성 시 Factory도 함께 생성.
</laravel-boost-guidelines>
