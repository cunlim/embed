...
| 인프라 | PostgreSQL 15 + pgvector, Redis, Ollama, Docker | `docs/ADR.md` 참조 |
...
Next.js 관련 작업은 호스트에서 직접 실행하지 말고 반드시 `cl_embed_nextjs` 컨테이너 내부에서 실행 (node_modules 권한 문제 방지).
...
- **Feature Spec 3축** — 새 기능 구현 전 이 3개 문서의 요구사항 일관성을 반드시 확인한다:
  ...
  - [`docs/ADR.md`](docs/ADR.md) — 인프라·아키텍처 결정 (How), 컨테이너 구성, 라우팅
