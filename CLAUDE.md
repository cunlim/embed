...
- **tinker 쓰기 권한 오류** — `useradd -m`으로 홈 디렉토리(`/home/appuser`)를 생성하므로 psysh 설정 디렉토리 권한 문제는 해결됨.
...
Laravel 컨테이너(`cl_embed_laravel`) 내부에서 실행되는 3개의 프로세스가 각각 별도 파일에 기록합니다. Docker Compose volume `./laravel/volume/log:/home/appuser/log/php`로 호스트에 마운트됩니다.
