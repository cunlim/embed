...
## [소개]
- 3년 이상 스타트업에서 풀스택 웹 개발자로 근무하며, Git·Docker·Vue 등 최신 기술 도입과 서비스 성능 최적화에 주도적으로 기여했습니다. 문제 해결 중심의 개발 경험과 빠른 적응력을 바탕으로, 귀사의 성장에 실질적인 가치를 더할 자신이 있습니다.
- 신규제품의 프로토타이핑, 기존제품의 고도화를 위해 요구사항 수집, 기술 검토, 성능 최적화, 다양한 방안 제안 등 다양한 업무를 서버 운영부터 프로젝트 개발까지 전담한 실무 경험이 있습니다.
- 기술의 진보에 따라 모던하면서도 안정적인 기술을 적극 도입해보고 있습니다. 예로 회사 업무 중 필요에 따라 Git, Docker, Vue.js, Xdebug 를 도입하여 개발 프로세스 및 배포 환경 개선했었고 개인 프로젝트에서는 인프라 현대화, AI 개발, 자동화배포 구축 등 업무 자동화를 시도해보았고 특히 AI 관련 정보는 트렌드를 적극적으로 따라가려 합니다.
- 2022년 8월 처음으로 한국에 입국했고 원활한 한국어 기반의 협업이 가능하며, F-4 비자 소지자로 고용 절차상 제약이 없습니다.
...
| | |
| --- | --- |
| **운영** | Linux(cloud server) + Docker + GitHub Actions 자동화배포 |
...
- **주요 성과:**
  * **아키텍처 개편 및 인프라 구축:** Git, Docker, Certbot, Composer, Xdebug 도입
    - **Git 도입:** 작업에 대한 책임을 분명하게 하고, 이슈 발생 시 작업 history 추적하는 정밀도와 속도가 90% 이상 향상, 큰 작업량의 코드를 배포환경에 적용하는 안전도와 속도 50% 향상.
    - **Docker 도입:** 각 실행 환경을 안전하게 격리하여 관리를 쉽게 하고, 외부 개발자한테 volume 을 통하여 안전하게 코드를 노출할수 있게 함.
    - **Certbot 도입:** 원스톱으로 SSL 인증 신청, 매일 새벽 자동 갱신하는 인프라를 구축하여 꼭 필요하지 않은 작은 프로젝트에도 쉽고 안정적으로 https 적용
    - **Composer 도입:** 바닐라 PHP 에 라이브러리를 수동으로 업로드 하여 path 불일치 등 이슈를 튜닝하던 불편함으로부터 의존성을 체계적으로 관리
    - **Xdebug 도입:** if is_admin 안에 print 하는 디버깅을 위한 코드가 너무 많고 웹페이지에 화면이 깨지는것을 감안하여 출력해보던 불편함으로부터 실행 타임라인과 실행된 모든 변수의 값을 추적할수 있고 실행 시간까지 알수 있는 편리함을 얻을수 있었습니다.
  * **업무요청에 맞게 신규 기술 도입:** PyPDF2(Python), Express(Node.js), Chrome Extension, Vue(CDN) C# terminal app 도입
    - **PyPDF2 도입:** api 로 받아온 PDF 들을 주문별로 합병하여 출력하는데에 편리함 30% 향상.
    - **Express 도입, Chrome Extension 개발:** 크롤링 수요에 맞춰 Express + Puppeteer 로 서버를 구축했지만 다양한 사이트의 캡챠를 뚫어야 하는 비효율을 극복하기 위해 Chrome Extension 으로 전환, 크롤링 결과를 parsing 하는 JavaScript 코드는 90% 재활용.
    - **Vue(CDN) 도입:** jQuery 만으로 DOM 컨트롤 하는 프로젝트에 데이터와 UI 의 복잡도가 큰 페이지 작업요청을 작업속도와 유지보수성을 고려하여 MVVM 을 지원하는 React 와 Vue 중 하나를 도입하려고 생각했고 PHP 에 익숙한 동료분들을 고려하여 CDN 방식의 Vue 를 도입.
    - **C# terminal app 신규 개발:** 자체 서비스 웹페이지에서 버튼을 클릭하여 외부개발자가 개발한 C# 프로그램을 부팅하는 동시에 데이터를 전달하는 기능 프로토타입을 작업하여 해당 기능 실현 가능성을 제시하여 외부개발자한테 전달.
  * **협업 인프라 구축:** Git, [VSCode + Remote SSH + saveBackup extension + 저장로그 통계 서비스 개발], 신규 기술 문서화
    ...
    - **VSCode + saveBackup extension:** 협업하면서 부주의로 파일이 손실 되어도 저장한 파일은 백업되는것 덕분에 갈등, 의심 20% 완화
    - **VSCode + saveBackup extension + 저장로그 통계 서비스:** 개발자의 작업 패턴을 추적하기 위해 계정별, 프로젝트별로 saveBackup 으로 생성된 파일들을 통계하는 페이지 개발하여 불필요한 오해 해소.
  * **DB 백업 및 로그:** DB 자동 백업(매일), DB 구조 변경 history 자동 기록(매시간) 으로 데이터 유실 사고 0건 유지
    - vanilla PHP 인지라 DB 마이그레이션 기록이 Git history 에 필수로 기록되지 않아 DB 구조를 매시간마다 dump 하여 결과를 Git 으로 관리. src 작업 과정과 DB 구조 변경 기록을 매칭할수 있게 됨.
  * **성능 개선:** slow query, 코드블록 별 실행시간 추적을 통하여 병목지점을 찾아내어 튜닝
    - slow query 로그, Xdebug, 시간 분석 함수를 사용하여 병목지점을 찾아냄. 병목 지점이 SQL 실행이면 explain 을 통한 분석, 적절한 인덱스 추가, SQL 조회방식 수정 등 방법으로 튜닝
