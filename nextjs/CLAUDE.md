...
`app/` (App Router), `components/` (ui/는 shadcn 자동 생성), `hooks/`, `lib/`. 디렉토리 상세는 `find nextjs/app nextjs/components -type f`로 확인.
...
Next.js 16 (App Router), React 19, TypeScript 5, Tailwind CSS v4, shadcn/ui (`base-nova`), next-themes, lucide-react, vitest + React Testing Library. 버전은 `package.json` 참조.
...
- **인증 가드 패턴 (admin)** — `useAuth()`의 `isLoading`으로 사용자 로딩 완료까지 대기. `authorized` 상태 플래그로 권한 확인 전까지 `return null`하여 admin 내용이 전혀 렌더링되지 않게 함. 관리자 확인은 `isAdmin(userId)` from `@/lib/utils` 사용 (인라인 `id === 3` 금지). 비로그인 → `router.replace("/login?redirect=/admin")`. 로그인 + 비관리자 → `router.back()`.
- **`isAdmin(userId: number)` — `@/lib/utils`** — 관리자 ID `3` 중앙 관리. admin/page.tsx, auth-buttons.tsx 등에서 인라인 ID 비교 대신 사용. ID 변경 시 한 곳만 수정.
