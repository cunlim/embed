...
- **Turbopack middleware 소스맵 디버깅** — Next.js 16의 middleware는 Edge Runtime으로 실행되며 Turbopack이 `server/edge/chunks/`에 번들링. 소스맵 경로가 `turbopack:///[project]/*` 프리픽스를 사용하므로, `launch.json`의 `sourceMapPathOverrides`에 `"turbopack:///[project]/*": "${workspaceFolder}/*"` 추가 필수. 미적용 시 middleware.ts breakpoint가 동작하지 않음.
