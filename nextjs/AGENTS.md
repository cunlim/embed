...
- **`loadCategories` `hierarchyLang` 파라미터 전파** — `loadCategories` 8번째 인자 `hierarchyLang`은 분류선택 모드에서 백엔드 `hierarchy_lang` 쿼리 파라미터로 전달. `hierarchyLangRef.current`로 최신값 읽기. **⚠️ `handleKeywordSearch`에서는 전달 금지** — 검색 모드의 키워드 검색은 모든 언어 컬럼에서 부분 검색(`%검색어%`)이어야 하므로 `hierarchy_lang`을 보내면 prefix 매칭(`검색어>%`)이 적용되어 결과가 없음. `handleStepsChange`·페이지 변경·폴더/삭제 완료 핸들러 등 분류선택 컨텍스트가 유지되는 호출 지점에서만 전달.
