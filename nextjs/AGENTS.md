...
- **async batch 진행률 `flushSync` 패턴** — 다수 API를 순차 호출하는 배치 작업(선택 처리, 삭제 등)에서 진행률이 실시간 갱신되지 않는 현상은 React 19가 `await` 경계를 넘어 상태 업데이트를 배칭하기 때문. `import { flushSync } from "react-dom"`으로 **루프 내 모든 `setProgress` 호출**을 `flushSync(() => setProgress(...))`로 감싸야 함. 일부만 감싸면 감싸지 않은 업데이트(예: `completedSteps` 증가)는 batching되어 렌더 누락 → progress bar가 중간에 멈춤. Phase 1(수집)→Phase 2(실행)로 나누고 진행률을 0-50%/50-100%로 분할 표시. **빈 카테고리 보정**: 확인만으로 완료된 카테고리(처리할 step 없음)는 Phase 1에서 전체 할당(10%)을 즉시 부여(`emptyCategories` 카운터) → Phase 2에서 마지막 step 완료 시 갑자기 튀는 현상 방지. 상세: `task-execution.tsx` 참조.
- **`fetchCategoryTranslations` `noPreview` 옵션** — `noPreview: true` 전달 시 `?no_preview=true` 쿼리 파라미터로 임베딩 벡터(preview) 제외. task-execution 등 배치 확인용 호출에서 응답 크기 절약. 상세: `api.ts`·`task-execution.tsx` 참조.
