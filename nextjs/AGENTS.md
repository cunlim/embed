...
- **헤더의 아이콘 버튼은 좁은 폭에서 아이콘 우선** — 텍스트 라벨은 `sm` 이상에서만 표시하고, 작은 폭에서는 `h-7 w-7` 수준으로 줄여 정사각형 비율을 유지한다.
- **목록의 상태 아이콘은 독립 컬럼 유지** — 좁은 폭에서 상태 아이콘이 카테고리명 아래로 떨어지지 않게 모바일 카드도 3열 그리드로 유지하고, 테이블에는 불필요한 `min-w-*`를 추가하지 않는다.
...
> 인프라·Docker·CI/CD 관련 이슈는 루트 [`AGENTS.md`](../AGENTS.md)의 "알려진 이슈" 섹션도 함께 참조할 것.
...
- 프로젝트 루트: [`/AGENTS.md`](../AGENTS.md)
- 백엔드 가이드라인: [`laravel/AGENTS.md`](../laravel/AGENTS.md)
