...
- **`loadCategories` `searchLang` 파라미터 전파** — `loadCategories` 8번째 인자 `searchLang`은 분류선택 모드에서 백엔드 `search_lang` 쿼리 파라미터로 전달. `hierarchyLangRef.current`로 최신값 읽기. `handleKeywordSearch`·`handleStepsChange`·페이지 변경·폴더/삭제 완료 핸들러 등 모든 `loadCategories` 호출 지점에서 `keywordRef.current`가 비어있지 않으면 전달. 빈 키워드(`""`)일 때는 전달 불필요.
