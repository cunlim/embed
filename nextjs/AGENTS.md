...
- **async batch 진행률 `flushSync` 패턴** — 다수 API를 순차 호출하는 배치 작업(선택 처리, 삭제 등)에서 진행률이 실시간 갱신되지 않는 현상은 React 19가 `await` 경계를 넘어 상태 업데이트를 배칭하기 때문. `import { flushSync } from "react-dom"`으로 **루프 내 모든 `setProgress` 호출**을 `flushSync(() => setProgress(...))`로 감싸야 함. 일부만 감싸면 감싸지 않은 업데이트는 batching되어 렌더 누락. **task-execution 구조**: Phase 1(확인)은 `POST /api/categories/batch-status` 단일 호출로 대체됨. `steps[]` 파라미터로 checkbox 상태를 전달하면 서버에서 missing_steps를 계산. progress bar는 Phase 2(run-step 실행)만 100%로 표시. 상세: `task-execution.tsx`·`laravel/AGENTS.md` 참조.
- **`fetchCategoryTranslations` `noPreview` 옵션** — `noPreview: true` 전달 시 `?no_preview=true` 쿼리 파라미터로 임베딩 벡터(preview) 제외. 카테고리 모달 상세 조회에서 사용. 배치 작업은 `batch-status` API로 대체됨. 상세: `api.ts`·`useCategoryDetail.ts` 참조.
