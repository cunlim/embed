...
describe("useCategories", () => {
  ...
  describe("loadCategories", () => {
    it("성공 시 categories를 갱신하고 isLoading이 false로 돌아온다", async () => {
      ...
      expect(result.current.isLoading).toBe(false);
      expect(result.current.error).toBeNull();
      expect(mockGetCategories).toHaveBeenCalledWith("token", 1, 20, undefined, undefined, undefined, undefined, undefined, undefined);
    });
  ...
  describe("addCategory", () => {
    it("성공 시 createCategory API를 호출하고 목록을 갱신한다", async () => {
      ...
      expect(mockCreateCategory).toHaveBeenCalledWith("의류>여성의류>원피스", "token", undefined, undefined, undefined, undefined, undefined);
      expect(result.current.categories).toEqual([mockCategory]);
      expect(result.current.isLoading).toBe(false);
      expect(result.current.error).toBeNull();
    });
