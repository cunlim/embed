...
**결정**: 외부 API(`/api/v1/search`) 인증에 사용되는 API 키는 SHA-256 해시로 저장한다. 생성 시 평문 키를 사용자에게 1회 반환하고, DB에는 `key_hash`(SHA-256 hex, 인덱스)와 `key_prefix`(표시용 10자)만 저장한다. Quota 차감은 `WHERE api_quota_remaining > 0` 조건을 포함한 원자적 SQL 연산으로 수행하여 동시 요청으로 인한 TOCTOU 경쟁 조건을 방지한다. 내부 API(`/api/recommend`)와 외부 API 모두 모든 사용자(관리자 포함)에게 quota를 차감한다.
