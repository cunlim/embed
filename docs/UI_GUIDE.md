...
- **목적**: 카테고리 CRUD, 일괄 번역 트리거, 개별 카테고리 번역/임베딩 실행, 시스템 관리
...
- **디자인 방향**:
  ...
  - **카테고리별 개별 번역/임베딩 실행** (아래 상세 명세 참조)
  - 일괄 번역: 언어 선택 + "전체 번역 실행" 버튼 + 진행률 바
  - 빈 상태: "등록된 카테고리가 없습니다"
  - 에러: inline error + retry button

#### 6.4.1 카테고리별 개별 "번역 → 임베딩" 실행

각 카테고리 테이블 행 우측에 **"번역 실행" 버튼**(Play 아이콘, `variant="ghost" size="icon"`)을 배치한다. 버튼 클릭 시 해당 카테고리 하나에 대해 번역→임베딩 파이프라인이 실행된다.

**파이프라인 5단계** — 하나의 카테고리에 대해 총 5회의 작업이 순차적으로 실행된다:

| 순서 | 단계 | 언어 | 작업 내용 |
|------|------|------|-----------|
| 1 | 번역 | zh | 한국어 원문을 중국어로 번역 (Ollama `translategemma:4b`) |
| 2 | 번역 | en | 한국어 원문을 영어로 번역 (Ollama `translategemma:4b`) |
| 3 | 임베딩 | ko | 한국어 원문을 벡터화 (Ollama `bge-m3:latest`, 1024차원) |
| 4 | 임베딩 | zh | 1단계에서 번역된 중국어 텍스트를 벡터화 |
| 5 | 임베딩 | en | 2단계에서 번역된 영어 텍스트를 벡터화 |

**진행률 표시 방식**:
- 버튼 클릭 시 해당 테이블 행이 **확장 가능한 프로그레스 패널**로 전환된다 (또는 행 하단에 인라인 프로그레스 바가 나타난다).
- 각 단계는 개별 WebSocket 이벤트(`category.{categoryId}.progress`)로 진행 상황을 수신하며, 완료된 단계는 체크 아이콘, 진행 중인 단계는 스피너, 대기 중인 단계는 회색 점으로 표시한다.
- 5단계 모두 완료 시 "완료" 상태로 2초간 표시 후 자동으로 닫힌다.
- 실패한 단계가 있으면 해당 단계에 빨간색 X 아이콘과 "재시도" 버튼을 표시한다.

**버튼 상태**:
- **진행 가능**: 해당 카테고리의 파이프라인이 실행 중이 아닐 때 — Play 아이콘, 툴팁 "번역 실행"
- **진행 중**: 이미 파이프라인이 실행 중일 때 — `animate-spin` 로더 아이콘으로 교체, 버튼 disabled
- **완료**: 모든 임베딩이 존재할 때 — Play 아이콘 유지, 툴팁 "번역 재실행" (언제든 재실행 가능)
