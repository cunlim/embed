...
`/admin` 관리자 전용 페이지를 생성하라. **이 페이지는 로그인한 사용자만 접근할 수 있다.** 비로그인 사용자가 접근하면 `/login`으로 리다이렉트해야 한다. PRD §3.4에 따라 카테고리 추가 기능은 로그인한 사용자에게만 노출된다. "관리자"란 `/admin` 접근 권한이 있는 로그인 사용자를 의미하며, 별도의 역할(Role) 구분은 없다.
